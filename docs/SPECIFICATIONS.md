...
- **Lists (Catalog, Installed):** `↑↓`/`j k` move; the detail pane updates live as the selection
  moves; `/` focuses an incremental, case-insensitive filter (live; `Esc` clears it and refocuses
  the table); `r` refresh. `Enter` is the "open" action — on Catalog it focuses the detail pane
  (the item isn't runnable yet), on Installed it runs the app (see below); the detail pane stays
  reachable via `Tab`.
